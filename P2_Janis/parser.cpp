...
node* program() {
	...
	// THIS IS USED AGAIN ON mStat(), because there is another *possible* empty production
			...
			if (tk.tokenString.compare(":=") == 0) {
				...
				if (tk.tokenType == INT_TOKEN) {
					...
					if (tk.tokenString.compare(";") == 0) {
						temp->tkFive = tk;
						tk = scan();
						temp->childOne = vars(); // was just vars()
						return temp; // will retun the node here eventually (ignore block for now)
					}
					else {
						errors("Integer", ";");
					}
...
node* stat() {
	...
	// check at the end of this assigment if this is necessary or not
	else {
		return temp;
	}	
}
...
*/
node* in() {
	// add token to node HERE PLEASE DEAR LORD
	node* temp = newNode("in");	
	// before I get the next token
	temp->tkOne = tk;
	tk = scan();
	if (tk.tokenType == ID_TOKEN) {
		temp->tkTwo = tk;
