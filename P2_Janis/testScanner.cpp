...
/	   also initializes the scanner, scans the file for input,  /
/	   after the scanner (assuming no scanner error was found), /
/      the parser() gets initialized.                           /
/      The parser checks to make sure a valid parse tree        /
/      exists for what was passed in, or gives a scanner        /
/      error.                                                  */
/***************************************************************/
...
int main(int argc, char *argv[]) {
	...
	/* Instead of printing scanner tokens, this now prints a parse tree (or a parser error) assuming
		that there was no scanner error or problems with the file
	*/
	initScan(fileName);
	root = parser();
